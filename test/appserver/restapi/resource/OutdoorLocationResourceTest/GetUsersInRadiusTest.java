...
public class GetUsersInRadiusTest {
    ...
    public void testCorrectData() {
        Response response = target.path("locations/outdoor/users/"+usernames+"/radius")
                .queryParam("center", coordinates1).queryParam("radius", 5).request().header(token_string, token).get();
        int status = response.getStatus();
        assertEquals(200, status);
    }

    @Test
    public void testResponseOnNegativeRadius() {
        Response response = target.path("locations/outdoor/users/"+usernames+"/radius")
                .queryParam("center", coordinates1).queryParam("radius", -1).request().header(token_string, token).get();
        int status = response.getStatus();
        assertEquals(400, status);
    }

    @Test
    public void testResponseOnRadiusIsZero() {
        Response response = target.path("locations/outdoor/users/"+usernames+"/radius")
                .queryParam("center", coordinates1).queryParam("radius", 0).request().header(token_string, token).get();
        int status = response.getStatus();
        assertEquals(400, status);
    }
    ...
    public void testCoordinateOutOfBounds() {
        Response response = target.path("locations/outdoor/users/"+usernames+"/radius")
                .queryParam("center", "181.004963;9.852982")
                .queryParam("polyCoordinate", 5).request().header(token_string, token).get();
